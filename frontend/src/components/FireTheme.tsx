...
export default function FireTheme({ isActive }: FireThemeProps) {
  ...
  return (
    <>
      {/* Soft warm glow outline effect - minimal aesthetic */}
      <motion.div
        ...
        exit={{ opacity: 0 }}
        style={{
          boxShadow: "inset 0 0 100px 30px rgba(249, 115, 22, 0.15)",
        }}
      >
        <motion.div
          ...
          animate={{
            boxShadow: [
              "inset 0 0 100px 30px rgba(249, 115, 22, 0.1)",
              "inset 0 0 120px 40px rgba(249, 115, 22, 0.2)",
              "inset 0 0 100px 30px rgba(249, 115, 22, 0.1)",
            ],
          }}
          transition={{
      ...
      <div className="fixed inset-0 pointer-events-none z-[59] overflow-hidden">
        <AnimatePresence>
          {particles.map((particle) => (
            ...
            >
              <span style={{ fontSize: `${particle.size}rem` }}>🔥</span>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
